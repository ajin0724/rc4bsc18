...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run):
    ...
    if model_flag == 'resnet18':
        model =  resnet18(pretrained=True, num_classes=n_classes) if resize else ResNet18(in_channels=n_channels, num_classes=n_classes)
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=gamma) #learning rate를 일정 시간마다 바꿔주는 것 
...
def test(model, evaluator: Evaluator, data_loader, task, criterion, device, run, save_folder=None):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(data_loader):
            ...
            if len(data_loader) == 27:
                if save_folder:
                    for i in range(len(inputs)):
                        original_image = transforms.ToPILImage()(inputs[i].cpu())
                        file_path = os.path.join(save_folder, f'image_{batch_idx * data_loader.batch_size + i}_original.png')
