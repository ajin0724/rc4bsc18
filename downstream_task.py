...
def main(data_flag, output_root, num_epochs, gpu_ids, batch_size, download, model_flag, resize, as_rgb, model_path, run):
    ...
    if model_path is not None:
        ...
        # 1. define the dino architecture
        dino = nn.Sequential(OrderedDict(list(resnet18().named_children())[:-1]))
        dino = nn.DataParallel(dino)

        # 2. load the pre-trained dino
        dino.load_state_dict(torch.load(model_path, map_location=device), strict=True)
        dino = dino.module
            
        # 3. modify keys
        for key in dino.state_dict().keys():
            if 'module.' in key:
                dino[key.replace('module.', '')] = dino.pop(key)
            
        # 4. add new fc layer
        dino.fc = nn.Linear(in_features=512, out_features=n_classes)

        # 5. assign to our model
        model.load_state_dict(dino.state_dict(), strict=False)
    ...
    # 1. initialize the early stop counter
    ...
    for epoch in trange(num_epochs):  
        ...
        # 2. early stop if the counter >= 20
        early_stop_counter += 1
        if early_stop_counter >= 20:
            break

        # 3. initialize the counter if the val_loss is less than best_loss
